...
class TritonServingClient(ModelServingClient):
    ...
    def set_client(self):
        try:
            ...
            print('set triton server on {}'.format(url))
    ...
    def set_inputs(self, inputs_node: list):
        ...
        for node in inputs_node:
            # input_tensor = grpcclient.InferInput(node.node_name, node.node_shape, node.node_type)

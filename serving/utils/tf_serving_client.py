...
class TFServingClient():
    ...
    def inference(self, name:str, configs: dict, _old_feed_map=None, _old_version=None):
        try:
            outputs = self._inference(name, configs, _old_feed_map, _old_version)
        except Exception as err:
            logger.error(err)
            raise exception.TFServingError
        return outputs

    def _inference(self, name: str, configs: dict, _old_feed_map=None, _old_version=None):
        ...
        # for old
        if _old_feed_map:
            return self.inference_old(name, configs, _old_feed_map, _old_version)
    ...
    def inference_old(self, name, signature_name, feed_map, version=None):
        """a simple wrapper for predict

        Returns:
          a PredictResponse
        """
        request = self.set_request(name, signature_name, feed_map, version)
        results = self.predict(request)
        return results